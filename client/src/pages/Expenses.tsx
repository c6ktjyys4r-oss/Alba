...
const CATEGORIES = ["Salaries","Rent","Utilities","Supplies","Maintenance","Marketing","Other"];

const defaultForm = { branchId:"", category:"Salaries", description:"", amount:"", date:"", reference:"" };
...
export default function Expenses() {
  ...
  const { data: expenses = [], isLoading } = trpc.expense.list.useQuery({ dateFrom, dateTo, branchId:branchFilter?Number(branchFilter):undefined });
  const { data: branches = [] } = trpc.branch.list.useQuery();

  const createMutation = trpc.expense.create.useMutation({
    onSuccess: () => { utils.expense.list.invalidate(); toast.success("Expense added"); setDialogOpen(false); setForm({...defaultForm}); },
    onError: (e) => toast.error(e.message),
  });
  const updateMutation = trpc.expense.update.useMutation({
    onSuccess: () => { utils.expense.list.invalidate(); toast.success("Expense updated"); setDialogOpen(false); },
    onError: (e) => toast.error(e.message),
  });
  const deleteMutation = trpc.expense.delete.useMutation({
    onSuccess: () => { utils.expense.list.invalidate(); toast.success("Expense deleted"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const openEdit = (r: any) => {
    setEditId(r.id);
    setForm({ branchId:r.branchId?String(r.branchId):"", category:r.category||"Other", description:r.description||"", amount:String(r.amount||""), date:r.date||"", reference:r.reference||"" });
    setDialogOpen(true);
  };
  ...
  const total = expenses.reduce((sum: number, r: any) => sum + Number(r.amount||0), 0);

  return (
    <div className="p-4 lg:p-6">
      <PageHeader title={t("accounting.expenses")} subtitle={`${expenses.length} ${t("common.records")}`}
        actions={<Button onClick={openCreate} size="sm" className="gap-2"><Plus size={16}/>{t("accounting.addExpense")}</Button>} />

      <div className="grid grid-cols-2 gap-4 mb-5">
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-red-100 flex items-center justify-center"><TrendingDown size={18} className="text-red-600"/></div>
            <div>
              <p className="text-xs text-slate-500">{t("accounting.totalExpenses")}</p>
              <p className="text-xl font-bold text-red-600">{total.toLocaleString()} {t("common.currency")}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center"><TrendingDown size={18} className="text-slate-600"/></div>
            <div>
              ...
              {isLoading ? Array.from({length:5}).map((_,i)=>(
                ...
                return (
                  <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3"><span className="text-xs bg-red-50 text-red-700 px-2 py-0.5 rounded-full">{r.category}</span></td>
                    ...
                    <td className="px-4 py-3 font-semibold text-red-600">{Number(r.amount).toLocaleString()} {t("common.currency")}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editId?"Edit Expense":t("accounting.addExpense")}</DialogTitle></DialogHeader>
