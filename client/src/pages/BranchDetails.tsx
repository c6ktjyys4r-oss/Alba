import { useParams, useLocation } from "wouter";
  ...
  import { ChevronLeft, MapPin, Phone, User2, Users, Crown, Stethoscope, HeartPulse, Briefcase, UserCheck } from "lucide-react";
  ...
  /** Return the first department that matches the visual group name. */
  function findDeptForGroup(groupName: string, departments: any[]): any | null {
  ...
  export default function BranchDetails() {
    ...
    empList.forEach((e) => {
      grouped[classifyEmployee(e, deptList)].push(e);
    });
    ...
    return (
      <div className="p-4 lg:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            {
              icon: <User2 size={16} className="text-blue-600" />,
              bg: "bg-blue-50",
              label: "Manager",
              value: (branch as any).managerName || "—",
            },
            {
              icon: <MapPin size={16} className="text-green-600" />,
              bg: "bg-green-50",
              label: "City / Address",
              value: (branch as any).address || "—",
            },
            {
              icon: <Phone size={16} className="text-amber-600" />,
              bg: "bg-amber-50",
              label: "Phone",
              value: (branch as any).phone || "—",
            },
            {
        ...
        {empList.length === 0 ? (
          ...
          <div className="space-y-8">
            {GROUP_ORDER.map((groupName) => {
              ...
              // ── Direct Manager data for this group ──────────────────────
              ...
              const directMgrEmp = directMgrId
                ? empList.find((e) => e.id === directMgrId)
                : null;
                ...
                <section key={groupName}>
                  ...
                  {!isMgr && (
                    <div className="flex items-center gap-2 mb-4 text-xs">
                      <UserCheck
                        size={13}
                        className={isDoctors ? "text-amber-500" : "text-slate-400"}
                      />
                      <span className="text-slate-500 shrink-0">Direct Manager:</span>

                      {isDoctors ? (
                        /* Doctors → always shows Branch Manager, read-only */
                        branchManager ? (
                          <span className="inline-flex items-center gap-1 font-medium text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">
                            ...
                            <span className="text-amber-400 font-normal">· auto</span>
                          </span>
                        ) : (
                          <span className="text-slate-400 italic">No Branch Manager assigned</span>
                        )
                      ) : dept ? (
                        /* Nursing / Administrative → editable dropdown */
                        <select
                          className="text-xs border border-slate-200 rounded-lg px-2 py-1 bg-white text-slate-700 focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-100 max-w-[220px]"
                          value={directMgrId ?? ""}
                          onChange={(e) => {
                        ...
                        <span className="text-slate-400 italic">No department matched</span>
                      )}
                    </div>
                  )}
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {/*
                                Req 1: Branch Manager group → show "Branch Manager" only.
                                Other groups → show job title + department.
                              */}
                              {isMgr ? (
                            ...
                            {/* Status badge */}
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 mt-0.5 ${statusClasses(emp.status)}`}
                            >
                              {emp.status}
                            </span>
                          </div>
